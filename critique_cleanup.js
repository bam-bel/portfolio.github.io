(function(){
  function init(){
    ...
    style.textContent=`
      /* Editorial cleanup: typography + whitespace replace structural chrome. */
    ...
    /* Move filters into the first editorial block, directly below its header. */
    ...
    /* Replace the empty Career OS media placeholder with a real systems visual. */
    ...
    /* Remove duplicated Mestawet repeats; keep the two distinct applications. */
    ...
    /* Remove any fixed corner watermark that repeats the footer identity line. */
    ...
    /* Restore a living Color Atmosphere experiment. */
    ...
    if(swatch){
      ...
      function paint(){
        ...
        var accent=185+Math.round(v*25);
        var stone=212-Math.round(v*28);
        swatch.style.background='radial-gradient(circle at '+Math.round(lastX*100)+'% '+Math.round(lastY*100)+'%, rgba('+accent+',136,95,.95), transparent 42%), linear-gradient(135deg, #B9885F '+Math.round(v*100)+'%, #D4C4B7 100%)';
    ...
    /* Restore the KPI tree as a quiet expandable system, without button chrome. */
